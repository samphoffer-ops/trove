...
| **Skimlinks** | Affiliate link wrapping on outbound "Shop now" taps — monetization | `EXPO_PUBLIC_SKIMLINKS_PUBLISHER_ID` | `mobile/.env` | `mobile/src/lib/affiliate.ts` |
...
`discover-brands` (Exa) finds candidate domains → fed into `catalog-intake` (Anthropic judges + scrapes + Voyage embeds each product) in small batches → lands in the `brands`/`products` tables → Sam approves/rejects brands in the Table Editor. As of 2026-06-28 this runs automatically every Monday via `.github/workflows/discover-brands.yml` (also runnable on demand from the Actions tab) — the two functions are still never chained in a single request (see the comment at the top of `discover-brands/index.ts` for why), the workflow just calls them sequentially in small paced batches instead of a human doing it by hand. Once products are live, the `rank_products_for_user` Postgres function (migration 010) does the actual feed personalization at read time, using pgvector to compare each user's behavioral "taste vector" against product embeddings.

## Not yet connected

- **Skimlinks**: publisher application was rejected (2026-06-28) — `EXPO_PUBLIC_SKIMLINKS_PUBLISHER_ID` is unset, affiliate wrapping code exists but is dormant until a publisher account is approved (Skimlinks or an alternative network)
