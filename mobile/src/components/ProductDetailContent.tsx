...
interface Props {
  ...
  // rather than as a full-screen route — swaps "go back" for "close the
  // modal," and similar-item taps swap the modal's content in place
  // instead of pushing a new route, since there's no route to push to.
  onClose?: () => void;
}
...
export function ProductDetailContent({ productId, topInset = 0, bottomInset = 0, onClose }: Props) {
  ...
  function openProductInPlace(id: string) {
    // Inside the modal, swap content in place rather than closing and
    // reopening; otherwise defer to the normal modal-vs-route decision.
    ...
    <View style={styles.root}>
      <ScrollView contentContainerStyle={{ paddingBottom: 120 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.hero}>
          ...
          <Pressable style={[styles.backBtn, { top: topInset + 12 }]} onPress={() => (onClose ? onClose() : goBack('/(tabs)/feed'))}>
            <ChevronLeftIcon />
          </Pressable>
          <View style={[styles.heroActions, { top: topInset + 12 }]}>
...
const styles = StyleSheet.create({
  ...
  hero:       { width: '100%', aspectRatio: 3/4, maxHeight: 520, backgroundColor: Colors.stoneSoft },
